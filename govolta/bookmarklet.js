javascript: (() => {
  /* Seat totals per class (changes at cutoff date) */
  ...
  document
    .querySelectorAll("a.js_departure-date:not(.disabled)")
    .forEach((anchor) => {
      ...
      if (trainPricesStr) {
        try {
          ...
          const economyObj = trainPrices.find(
            (tp) => tp.comfort_class === "gv_economy",
          );
          const komfortObj = trainPrices.find(
            (tp) => tp.comfort_class === "gv_premium",
          );
          if (economyObj) {
          ...
          if (komfortObj) {
            komfort.price = komfortObj.day_price_full || null;
      ...
      const capacity = getCapacity(anchor.dataset.date);
      const economyIcon = getAvailabilityEmoji(
        economy.vacant_seats,
        capacity.economy,
      );
      const komfortIcon = getAvailabilityEmoji(
        komfort.vacant_seats,
        capacity.komfort,
      );
      ...
      anchor
        .querySelector(".day")
        .insertAdjacentHTML("beforeend", ticketInfoHtml);
    });
})();
